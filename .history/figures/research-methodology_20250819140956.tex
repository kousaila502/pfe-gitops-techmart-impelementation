...
% Phase 1 components (spread out horizontally)
\node[box] (p1baseline) at (-7,1.5) {Baseline\\Establishment};
\node[box] (p1control) at (-4,1.5) {Controlled\\Comparison};
\node[box] (p1failure) at (-1,1.5) {Failure\\Testing};
...
% Phase 2 components (spread out horizontally)
\node[box] (p2complex) at (1,1.5) {Complexity\\Normalization};
\node[box] (p2multi) at (4,1.5) {Multi-Service\\Testing};
\node[box] (p2hybrid) at (7,1.5) {Hybrid\\Integration};
...
% Key findings boxes (more horizontal spacing)
\node[box, fill=gitopsgreen!30, text width=2.5cm] (finding1) at (-7.5,-3) {
    \small \textbf{GitOps:}\\
    100\% automation\\
    37s recovery
};

\node[box, fill=gitred!30, text width=2.5cm] (finding2) at (-2.5,-3) {
    \small \textbf{Traditional:}\\
    2.3x faster builds\\
    Manual gates
};

\node[box, fill=gitorange!30, text width=2.5cm] (finding3) at (2.5,-3) {
    \small \textbf{Hybrid:}\\
    Zero overhead\\
    integration
};

\node[box, fill=gitopsblue!30, text width=2.5cm] (finding4) at (7.5,-3) {
