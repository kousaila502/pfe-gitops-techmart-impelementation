...
Figure \ref{fig:infrastructure-deployment-diagram} illustrates the comprehensive multi-platform infrastructure distribution strategy that enables both cost optimization and rigorous methodology comparison across different cloud environments.
...
Figure \ref{fig:argocd-deployment-sync} shows the real-time synchronization process that enables automatic deployment updates without human intervention, while Figure \ref{fig:argocd-deployment-history} demonstrates the comprehensive revision tracking capabilities that support instant rollback operations through Git-based version control.
...
Figure \ref{fig:heroku-deployment-logs} illustrates the streamlined deployment process that characterizes Traditional CI/CD approaches, with direct platform integration and minimal orchestration overhead.
...
Figures \ref{fig:postgresql-user-tables} and \ref{fig:postgresql-order-tables} showcase the sophisticated relational schema design supporting complex business transactions, while Figure \ref{fig:mongodb-collections} demonstrates the flexible document structure enabling variable product attributes. Figure \ref{fig:redis-cache-data} illustrates the high-performance in-memory data structures that support real-time cart operations and session management.
...
The TechMart platform successfully demonstrates both GitOps and Traditional CI/CD methodologies through a fully functional e-commerce interface that provides realistic business complexity for methodology evaluation. Figure \ref{fig:techmart-products-catalog} showcases the MongoDB-based flexible product catalog management, while Figure \ref{fig:techmart-product-details} demonstrates the detailed product information interface supporting comprehensive e-commerce functionality.

Figure \ref{fig:techmart-shopping-cart} illustrates the Redis-powered high-performance cart management system that maintains session state across distributed services. The authenticated user experience is shown in Figure \ref{fig:techmart-user-dashboard}, with comprehensive order management capabilities demonstrated in Figure \ref{fig:techmart-user-orders}.

Administrative functionality is presented through Figure \ref{fig:techmart-admin-dashboard}, which shows the comprehensive management interface, while Figures \ref{fig:techmart-user-management} and \ref{fig:techmart-order-management} demonstrate the detailed administrative controls for user and order management respectively.
...
Figure \ref{fig:github-actions-workflow-comparison} provides a comprehensive architectural comparison showing how both methodologies achieve deployment automation through different philosophical approaches, with GitOps emphasizing declarative state management and Traditional CI/CD focusing on imperative execution patterns.

The live automation processes are captured in Figure \ref{fig:github-actions-running}, demonstrating the sophisticated CI/CD pipeline execution across both methodologies. Figure \ref{fig:docker-images-registry} shows the container registry management strategy that supports both deployment approaches with research-specific tagging for experimental correlation and performance tracking.
...
The comprehensive database architecture demonstrates enterprise-grade polyglot persistence practices while supporting both operational requirements and experimental analysis through comprehensive monitoring and integration capabilities. Figure \ref{fig:database-architecture-diagram} illustrates the strategic distribution of database technologies across different services, showing how PostgreSQL supports transactional data requirements, MongoDB enables flexible catalog management, and Redis provides high-performance caching and session management across the distributed microservices architecture.
